	...
	public class CosmosDbOperation
	{
		public async Task Add()
		{
			using ( var client = new DocumentClient(new Uri("https://localhost:8081"), "C2y6yDjf5/R+ob0N8A7Cgv30VRDJIWEHLM+4QDU5DE2nQ9nDuVTqobD4b8mGGyPMbIZnqyMsEcaGQy67XIw/Jw=="))
			{
				...
				Console.WriteLine("\r\n>>>>>>>>>>>>>>>> Creating Collection <<<<<<<<<<<<<<<<<<<");
				var collectionDefinition = new DocumentCollection { Id = "weather", PartitionKey = new PartitionKeyDefinition(){Paths = new Collection<string>(){ "/weather/id" } }};
				var collection = await client.CreateDocumentCollectionIfNotExistsAsync(UriFactory.CreateDatabaseUri("SampleDb"),
